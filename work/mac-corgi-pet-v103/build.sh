...
/usr/bin/printf '{\n  "schemaVersion": 1,\n  "version": "%s",\n  "build": %s,\n  "releaseNotes": "修复 Windows 更新清单的中文编码解析并优化失败提示。",\n  "releases": [\n    {\n      "build": 7,\n      "notes": ["新增应用内在线更新"]\n    },\n    {\n      "build": 8,\n      "notes": ["优化“扭屁股”动作", "新增“无聊”动作"]\n    },\n    {\n      "build": 9,\n      "notes": ["优化更新提示与安装进度", "精简“待优化”动作", "调整“无聊”动作节奏", "新增 Windows 版本"]\n    },\n    {\n      "build": 10,\n      "notes": ["“无聊”动作改为连续播放 3 遍"]\n    },\n    {\n      "build": 11,\n      "notes": ["新增 Windows 在线更新", "双端更新能力保持一致"]\n    },\n    {\n      "build": 12,\n      "notes": ["修复 Windows 更新清单的中文编码解析", "优化更新失败提示"]\n    }\n  ],\n  "platforms": {\n%s\n  }\n}\n' \
  "$VERSION" "$BUILD" "$PLATFORMS_JSON" > "$OUTPUT_DIR/update.json"
/usr/bin/python3 -c 'import json, pathlib, sys; path = pathlib.Path(sys.argv[1]); data = json.loads(path.read_text(encoding="utf-8")); path.write_text(json.dumps(data, ensure_ascii=True, indent=2) + "\n", encoding="ascii")' "$OUTPUT_DIR/update.json"
echo "$APP_DIR"
